...
3. Prioritize findings in this order: correctness and regressions; security, privacy, authorization, and secret handling; data integrity, reliability, and performance; then compatibility, maintainability, and test coverage.
...
Report only findings that warrant action. Put the most serious issue first.
...
Use `critical` for data exposure/loss, privilege escalation, or widespread outage; `high` for likely functional or security regressions; `medium` for bounded, real defects; and `low` for minor defects with limited impact.
